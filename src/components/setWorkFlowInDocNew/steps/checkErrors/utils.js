...
export const extractProcessObjChecker = (
  ...
  const processObj = {
    company_id:
      currentUserDetails?.portfolio_info?.length > 1
        ? currentUserDetails?.portfolio_info.find(
    ...
    org_name:
      currentUserDetails?.portfolio_info?.length > 1
        ...
        : currentUserDetails?.portfolio_info[0]?.org_name,
  };
  // ('the documentToProcess is in util ', documentToProcess)
  // ('the documentProcessSteps is in util ', documentProcessSteps)
    ...
    return {
      error: `Please make sure you ${
        requiredProcessStepsKeys[
          requiredFieldKeys[
            pendingFieldsToFill.findIndex((field) => field === "field missing")
          ]
        ]
      } for each step`,
    };
  ...
  const documentMapMissingInStep = processObj.workflows[0].workflows.steps.map(
    (step, index) => {
      if (
        step.stepDocumentMap.length < 1 &&
        !step.skipStep &&
        documentProcessSteps[0].steps[index].stepRights === "add_edit"
      )
