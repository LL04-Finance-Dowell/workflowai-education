...
export default function DocumentDetailReport() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // ('API Response:', response.data.success);
    ...
    datasets: [
      {
        ...
        data: [
          ...
          documentReportData?.normality_analysis?.list1
            ?.slope_percentage_deviation || 0.25,
          documentReportData?.normality_analysis?.list1?.calculated_slope ||
            1.84,
        ],
    ...
    datasets: [
      {
        ...
        data: [
          documentReportData?.central_tendencies?.normal_dist?.mergedMean ||
            5.0,
          documentReportData?.central_tendencies?.normal_dist?.mergedMedian ||
            4.0,
          documentReportData?.central_tendencies?.normal_dist?.mergedMode
            ?.length || 4.0,
        ],
        backgroundColor: [
          "rgba(255, 99, 132, 0.6)",
  ...
  if (documentReportData && documentReportData.score_list) {
    ...
    promotersScores =
      (documentReportData.score_list.filter(
        (score) => score === 9 || score === 10
      ).length /
        scoreListLength) *
      100;
    passiveScores =
      ...
      100;
    DetractorsScores =
      (documentReportData.score_list.filter((score) => score > 0 && score <= 6)
        .length /
        scoreListLength) *
      100;
  }
