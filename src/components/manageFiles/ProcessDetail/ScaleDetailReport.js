...
export default function ScaleDetailReport() {
  ...
  const apiUrl =
    "https://100035.pythonanywhere.com/evaluation/evaluation-api/?report_type=scale";
    ...
    datasets: [
      {
        ...
        data: [
          ...
          scaleReportData?.normality_analysis?.list1
    ...
    datasets: [
      {
        ...
        data: [
          ...
          scaleReportData?.central_tendencies?.normal_dist?.mergedMode
            ?.length || 4.0,
        ],
        backgroundColor: [
          "rgba(255, 99, 132, 0.6)",
  ...
  if (scaleReportData && scaleReportData.score_list) {
    ...
    promotersScores =
      (scaleReportData.score_list.filter((score) => score === 9 || score === 10)
        .length /
        scoreListLength) *
      100;
    passiveScores =
      (scaleReportData.score_list.filter((score) => score === 7 || score === 8)
        .length /
        scoreListLength) *
      100;
    DetractorsScores =
      (scaleReportData.score_list.filter((score) => score > 0 && score <= 6)
        .length /
        scoreListLength) *
      100;
  }
