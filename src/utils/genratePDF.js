...
export const generatePDF = (pages, allScalesArray) => (
  <Document>
    {pages.map((element, index) => {
      ...
      return (
        <Page style={StyleSheet.create(pageSize)}>
          ...
          <View>
            {pageElementsArray.map((el, i) => {
              ...
              const parentStyleObject =
                convertInlineStylesToStyleObject(parentStyle);
              let childDiv;
              //Grab child div/button
              childDiv =
                Array.from(el.getElementsByTagName("button")).length > 0
              ...
              const childStyleObject =
                convertInlineStylesToStyleObject(childStyle);
              //
                    ...
                    {childStyleObject.hasOwnProperty("backgroundImage") ? (
                      <Image
                        style={StyleSheet.create({
                          width: childStyleObject["width"],
                          height: childStyleObject["height"],
                        })}
                        src={childStyleObject["backgroundImage"]}
                      />
                    ) : childDiv.getAttribute("class") === "newScaleInput" ? (
                      <Image
                        style={StyleSheet.create({
                          width: childStyleObject["width"],
                          height: childStyleObject["height"],
                        })}
                        src={childDiv.getAttribute("snapshot")}
                      ></Image>
                    ) : (
