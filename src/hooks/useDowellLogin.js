...
import { getUserInfo, getUserInfoOther } from "../features/auth/asyncThunks";
import {
  setId,
  setSessionId,
  updateUserDetail,
...
export default function useDowellLogin() {
  ...
  const handleUnauthorizedVerificationAccess = (
    passedUserDetail,
    passedSessionId,
    passedId = null,
    authorizedPortfolioName = null
  ) => {
    /**
    ...
    toast.info(
      authorizedPortfolioName
        ? `Please open this link with ${decodeURIComponent(
            authorizedPortfolioName
  ...
  const extractTokenFromURLAndNavigateToVerificationPage = (
    url,
    session_id,
    id = null,
    ...
    if (detailsConfigured) {
      ...
      authServices
        .getUserDetailOtherAsync({ session_id: session_id, product: product })
        .then((res) => {
          const fetchedUserDetails = res.data;

          window.sessionStorage.setItem(
            "userDetail",
            JSON.stringify(fetchedUserDetails)
          );
          dispatch(updateUserDetail(fetchedUserDetails));

          const userPortfolioName =
            fetchedUserDetails?.portfolio_info?.length > 1
              ? fetchedUserDetails?.portfolio_info.find(
          ...
          if (portfolio !== userPortfolioName) {
            handleUnauthorizedVerificationAccess(
              fetchedUserDetails,
              session_id,
              id,
              portfolio
          ...
          navigate(
            `/verify/${token}/?auth_role=${role}&user_type=${userType}&portfolio=${portfolio}&username=${username}&org=${orgName}&product=${encodeURIComponent(
              product
            )}`
          );
        })
    ...
    authServices
      ...
      .then((res) => {
        ...
        window.location.replace(
          `${auth_expo_url}/exportfolio?session_id=${session_id}&org=${orgName}&product=${encodeURIComponent(
            product
          )}&portfolio=${portfolio}&username=${username}&redirect_url=${
            window.location.origin
          }/100018-dowellWorkflowAi-testing/%23token~${token}~role~${role}~userType~${userType}~portfolio~${portfolio}~username~${username}~org~${orgName}~product~${product}~userDetailsConfigured~`
        );
        return;
      })
      .catch((error) => {
  ...
  const extractImportProcessIdFromURLAndNavigate = (url) => {
    ...
    const [processId] = [url.split("importProcessId~")[1]?.split("~")[0]];
  ...
  useEffect(() => {
    ...
    if (session_id) {
      ...
      if (id || localId) {
        dispatch(setId(id));
        if (
          currentLocation.includes("token~") &&
          currentLocation.includes("userDetailsConfigured~")
        )
          return extractTokenFromURLAndNavigateToVerificationPage(
            currentLocation,
            session_id,
            id,
            true
