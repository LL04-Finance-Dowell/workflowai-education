...
export class DocumentServices {
  ...
  detailDocument = (data, collection_id) => {
    ...
    if (data.document_state == "draft") {
      return httpDocument.get(
        `/documents/${data.collection_id}/link/?document_type=document`
      );
    ...
    return httpDocument.get(
      // `/documents/${data.collection_id}/link/?document_type=document`
      `/${data.collection_id}/list/?document_type=document`,
      {
        headers: {
          Authorization: "Bearer 1b834e07-c68b-4bf6-96dd-ab7cdc62f07f",
        },
  ...
  documentCloneReport = (documentId) => {
    return httpDocument.get(
      `/documents/${documentId}/link/?document_type=clone`
    );
  };

  getOrgDocumentReportsFinalized = (companyId, dataType, state, member) =>
    httpApiUrlV2.get(
      `metadata/${companyId}/organisations/?data_type=${dataType}&document_state=finalized&item_type=clone`
    );
  getOrgDocumentReports = (companyId, dataType, state, member) =>
    httpApiUrlV2.get(
      `metadata/${companyId}/organisations/?data_type=${dataType}&document_state=rejected&member=${member}&item_type=clone`
    );
}
