...
const Option = ({ id, className }) => {
  const titleFormat = () => {
    ...
    else if (className.includes(CONTAINER_INPUT))
      return `CONTAINER INPUT - ${id}`;
    ...
    else if (className.includes(NEW_SCALE_INPUT))
      return `NEW SCALE INPUT  - ${id}`;
    ...
    else if (className.includes(DROPDOWN_INPUT))
      return `DROPDOWN INPUT  - ${id}`;
...
const SelectAnsAndQuestion = ({
  ...
  const { questionAndAnswerGroupedData, setQuestionAndAnsGroupedData } =
    useStateContext();
  ...
  const currentElmId = document.querySelector('.focussedd div')?.id
    ? document.querySelector('.focussedd div')?.id
    : null;
  const elements = [
  ...
  const handleSelectAnswerHelper = (passedData) => {
    ...
    const currentDataAsQuestion = data.find(
      (elm) => elm.question === currentElmId
    );
    ...
    if (currentDataAsQuestion) {
      const newData = {
        ...currentDataAsQuestion,
        answers: [...currentDataAsQuestion.answers, id],
      };
      peformStatesUpdates(
        data.map((elm) => (elm.question === currentElmId ? newData : elm))
      );
    } else {
  ...
  const handleSelectAnswer = (multipleValues) => {
    ...
    const passedData = [...multipleValues].map((options) =>
    ...
    setQuestionAndAnsGroupedData((prevData) => {
      return prevData.map((item) => {
        if (item.question === currentElmId) {
          return {
            ...item,
            answers: item.answers.filter((item) => item !== ans),
          };
        }
        return item;
      });
      ...
      <Row>
        ...
        <Col>
          <select
            ...
            className='select border-0 bg-white rounded w-100 h-100 ques_sel'
            id='font-sizing'
          >
